...
module.exports = {
    ...
    save: async (req, res) => {
        const {
            ...
            is_popular: isPopular, multi_files: photos, is_primary_photo: primaryPhoto
        } = req.body;

        try {
            const photoId = req.files.map(async photo => {
                const year = (new Date()).getFullYear().toString();
                const month = ((new Date()).getMonth() + 1).toString();
                const oldPath = `uploads/temp/${photo.filename}`;
                const dir = `uploads/${year}/${month}`;
                const newPath = `${dir}/${photo.filename}`;
                ...
                const image = await Image.create({
                    ...
                    isPrimary: photo.filename === primaryPhoto
                });
                return image._id;
            });
            Promise.all(photoId)
